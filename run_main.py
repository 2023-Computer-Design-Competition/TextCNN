...
def load_dataset(text_field, label_field, args, **kwargs):
    train_dataset, dev_dataset = process_data.get_dataset('my_data', text_field, label_field)
...
train_iter, dev_iter = load_dataset(text_field, label_field, args, device=0, repeat=False, shuffle=True)
...
# args.gsm_vector = torch.from_numpy(process_data.vector_loader('my_data\\gsm_test.json'))
print('Parameters:')
...
# save vocab
joblib.dump(text_field, 'vocab\\text_field.pt')
joblib.dump(label_field, 'vocab\\label_field.pt')
